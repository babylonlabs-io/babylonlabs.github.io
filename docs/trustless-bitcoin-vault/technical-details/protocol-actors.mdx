...
The umbrella distinction matters because all three share co-signing duties during peg-in and participate in the protocol's challenge mechanism. The detailed sections below describe each role's specific responsibilities. For the canonical taxonomy in the architecture spec, see the Trustless Bitcoin Vaults Architecture document.

## Who runs these actors today

On the current public testnet release, the actors are operated by Babylon and a small set of partner organizations. Specific operator BTC public keys are listed on [Contract addresses](../testnet-info/contract-addresses.mdx).
...
* **Universal Challengers:** 3 operators protocol-wide (Altlayer, Babylon 0, Babylon 1). The UC set is a static registry; it is not planned to become permissionless.
...
An earlier protocol-enforced minimum commission floor is being removed. Commission will be set entirely per-VP without a protocol-enforced minimum. The portal UX surfaces VP commission rates clearly to depositors.
...
* **Fraud monitoring.** Anyone can monitor for fraudulent claims by running a watcher and checking Ethereum-side events. Broadcasting an on-chain challenge requires being a registered Universal Challenger in the current `ProtocolParams` set; the registry is governed at the protocol level and is not planned to become permissionless.
...
For the underlying protocol mechanics that connect these actors, see [Protocol architecture](./protocol-architecture.mdx). For definitions of the protocol-specific terms used throughout this page, see [TBV Glossary](../reference/glossary.mdx).
