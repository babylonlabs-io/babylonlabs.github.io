...
The [Babylon Trustless Bitcoin Vaults (TBV)](../start-here/what-is-tbv.mdx) protocol has one user role, the **depositor**, and four operator groups: Vault Providers, App Keepers, Universal Challengers, and a transitional Security Council. None of the operator groups has custody of a depositor's BTC. They coordinate setup, generate proofs, monitor claims, settle liquidations, or provide an emergency backstop. The depositor's recovery paths do not depend on trusting any single operator. This page describes what each actor does at the protocol level.

## What is a Vault Keeper?

BTCVault creation and redemption need a set of **Vault Keepers** that coordinate with each other. During setup, the Vault Keepers and the depositor pre-sign the spend paths that define how the BTC can be released, and each operator confirms readiness to perform its role. Operators with claim rights can receive the Bitcoin, but every claim is challengeable, so no single operator can unilaterally move or seize Bitcoin. The depositor is always an authorized claimer.

The protocol defines three types of operator that make up the Vault Keeper set:

* **Vault Provider (VP).** A claimer and challenger: it drives the standard redemption claim and can also challenge an invalid claim. One VP is chosen per vault at peg-in and is fixed for that vault's life.
* **Universal Challenger.** A challenger only: it monitors claims across every BTCVault, in every application, and challenges any that are invalid. It never claims a vault.
* **App Keeper.** An application-scoped claimer and challenger. In the Aave v4 integration, App Keepers act as Arbitrageurs during liquidation settlement, claiming a seized vault through direct redemption, and they can also challenge an invalid claim.

All three take part in vault creation through the pre-signing (BABE) ceremony. They are distinguished by their claim and challenge rights during redemption, not by who sets up the vault.
...
The Vault Provider is a **claimer and challenger**: it drives the standard redemption claim on Bitcoin and can also challenge an invalid claim. It takes part in vault creation alongside the other operator roles, drives peg-in setup, posts required data on-chain, and generates the zero-knowledge proofs used during redemption on Bitcoin.

### What the Vault Provider does

* **Peg-in coordination.** When a depositor submits a vault creation request, the Vault Provider detects the on-chain event and constructs the pre-signed transaction graph with the App Keepers and Universal Challengers committed to that vault.
...
App Keepers are **claimers and challengers**: in liquidation they claim a seized vault through direct redemption, and they can also challenge an invalid claim. They are application-scoped participants that take part in vault creation. In the [Aave v4 integration](./aave-v4-integration.mdx), App Keepers act as **Arbitrageurs** during liquidation settlement. That role is specific to the Aave application's design and may differ in future integrations.

### What the App Keeper does

* **Transaction graph participation.** During vault creation, each App Keeper constructs its own transaction graph and cross-signs with the Vault Provider. This gives multiple independent parties the pre-signed transactions needed for vault outcomes.
* **PegIn input signing.** Each App Keeper signs the Pre-PegIn HTLC spend as part of the off-chain protocol.
* **ACK submission.** Each App Keeper submits an on-chain acknowledgment confirming graph setup, signature collection, and verification of the Pre-PegIn confirmation depth.
* **Aave-specific liquidation settlement.** The Aave integration routes permissionless liquidation through a Liquidation Liquidity Provider (LLP); the BTCVault Swap Spoke is the default LLP. Liquidators receive instant WBTC liquidity from the LLP, while the seized vault enters escrow until a registered Arbitrageur acquires it and finalizes BTC redemption on Bitcoin. Only registered App Keepers can acquire escrowed vaults, because they must have pre-signed the vault's transaction graph at creation.
* **Challenge participation.** As holders of pre-signed transaction graphs and relevant artifacts, App Keepers can participate in the challenge mechanism if they detect an invalid claim against a vault they co-signed.

### Registration

App Keepers are registered per application through the vault registry. Each application maintains a versioned set of App Keeper key pairs (Ethereum + Bitcoin). A set version is immutable once active: rotation produces a new version that applies only to vaults created after the rotation. Vaults already in flight retain the set version live at their creation.

## Universal Challenger

Universal Challengers are **challengers only**: they challenge invalid claims but never claim a vault themselves. Like the other operator roles, they take part in vault creation. They are protocol-level fraud monitors whose role is to detect and challenge invalid proof submissions during vault redemption, regardless of which application a vault belongs to.
...
* **Peg-in setup.** Universal Challengers take part in vault creation, contributing the vault-specific commitments that make later challenge transactions valid on Bitcoin. The commitment is bound to the vault's specific Pre-PegIn HTLC output, which prevents cross-vault replay.
