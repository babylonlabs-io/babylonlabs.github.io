...
Universal Challengers are separate protocol-level fraud monitors. They challenge only and never claim a vault. All three operator roles — Vault Provider, Application Vault Keeper, and Universal Challenger — take part in vault creation through the pre-signing (BABE) ceremony; they are distinguished by their claim and challenge roles during redemption, not by who sets up the vault. Universal Challengers participate in the challenge mechanism across applications, but they are not application-scoped Vault Keepers.
