...
* **Application Vault Keeper (AVK).** An application-scoped claimer and challenger. In the Aave v4 integration, AVKs act as arbitrageurs during liquidation settlement, claiming a seized vault through direct redemption, and they can also challenge an invalid claim.

Universal Challengers are separate protocol-level fraud monitors. They challenge only and never claim a vault. All three operator roles — Vault Provider, Application Vault Keeper, and Universal Challenger — take part in vault creation through the pre-signing (BABE) ceremony; they are distinguished by their claim and challenge roles during redemption, not by who sets up the vault. Universal Challengers act across applications, so they are not application-scoped Vault Keepers.
...
Application Vault Keepers are **claimers and challengers**: in liquidation they claim a seized vault through direct redemption, and they can also challenge an invalid claim. They are application-scoped participants that take part in vault creation. In the [Aave v4 integration](./aave-v4-integration.mdx), AVKs act as **arbitrageurs** during liquidation settlement. That role is specific to the Aave application's design and may differ in future integrations.
