...
* `reorderVaults(bytes32[] permuted)`. Reorders the vaults in the position. The argument is a permutation of the position's vault IDs (`bytes32`), not indices; the order determines liquidation seizure order.
