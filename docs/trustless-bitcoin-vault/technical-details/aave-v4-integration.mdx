...
* **ERC-20-compatible, 8 decimals.** Matches Bitcoin's satoshi precision. 1 `vaultBTC` = 1 BTC.
...
* **Pricing.** The Babylon Core Spoke uses a Chainlink BTC/USD oracle to determine the USD value of `vaultBTC` for health factor calculations. Each unit corresponds to one satoshi of BTC locked in a vault. Liquidation settlement and fairness payment additionally use a Chainlink WBTC/USD oracle.
...
A position holds one or more vaults. When liquidation is triggered, the protocol walks the ordered vault list and seizes the minimum prefix needed to restore the health factor to the target health factor. Vaults at the front of the list are seized first.
