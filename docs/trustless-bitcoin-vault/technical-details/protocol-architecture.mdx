...
The Babylon Trustless Bitcoin Vaults (TBV) protocol has three layers: Bitcoin scripts and transactions, Ethereum contracts, and off-chain participant software. For the trust model, see [Safety & trust assumptions](../start-here/safety-and-trust-assumptions.mdx).

![TBV architecture: BTCVaults on Bitcoin, protocol and Aave v4 contracts on Ethereum, and off-chain protocol actors](/img/trustless-bitcoin-vault/confluence/technical-details/protocol-architecture/10.png)

## System components

### On Bitcoin

The Bitcoin-side vault is the **BTCVault**: the depositor's BTC in a Bitcoin Taproot output. Its spending paths cover the outcomes agreed at vault creation: redemption after withdrawal, redemption after liquidation, and challenge responses. Before BTC reaches the BTCVault output, it sits briefly in a **Pre-PegIn HTLC output**. That output gates activation on the depositor revealing a hashlock secret on Ethereum. It also includes a relative CSV refund path with a timelock of 2016 BTC blocks (~14 days) for the current public-testnet parameter version, so the depositor can recover BTC unilaterally if activation never completes.

The BTCVault output is locked by a pre-signed transaction graph. Every legitimate spend path is constructed and signed by the required participants at vault creation. After creation, no party can invent a new spend; Bitcoin will accept only the paths already encoded and signed.
...
* **Fee escrow**: collects the peg-in fee and distributes it to the Vault Provider, Universal Challengers, and App Keepers.
...
* **BTCVault Swap Spoke**: default Liquidation Liquidity Provider (LLP) for the Aave integration. It is registered as its own Aave Hub spoke for WBTC liquidity, distinct from the Babylon Core Lending Spoke for vault collateral.
...
* **Participants.** The Vault Provider, App Keepers, and Universal Challengers each run vault management software. Per-actor responsibilities are on [Protocol actors](./protocol-actors.mdx).
...
* **BABE setup.** The Vault Provider, App Keepers, and Universal Challengers run the BABE cut-and-choose protocol to establish garbled-circuit instances for the claim and challenge paths.
...
The contract verifies the hashlock, then notifies the registered application. For the Aave v4 integration, this deploys the depositor's position proxy if this is the first vault, creates the internal collateral record for the vault's BTC amount, and supplies it as collateral to the Babylon Core Lending Spoke.

Any party, typically the Vault Provider, observing the activation notification can construct the final PegIn witness using the revealed secret and the collected on-chain signatures, then broadcast the PegIn transaction on Bitcoin. Once confirmed, the BTC is in its BTCVault output. The PegIn transaction also produces a small depositor claim output, controlled solely by the depositor's key, that anchors the self-claim path.
...
* An application-specific liquidation seizes one or more vaults. For the Aave v4 integration, this is either the direct BTC redemption path for registered Arbitrageurs or the permissionless LLP-routed path that escrows the seized vault in the BTCVault Swap Spoke to receive WBTC.
...
* **Challenge window.** During the challenge window, any Vault Keeper committed to the vault, or the depositor using their artifacts, can dispute an invalid proof.
...
* A Vault Keeper committed to the vault, or the depositor, detects the invalid proof and replays it under the BABE input labels established during peg-in. They broadcast a challenge transaction on Bitcoin.
...
The same versioning applies to `tRefund`. The current value is 2016 BTC blocks (~14 days). Earlier vaults can use 432 BTC blocks (~3 days).
