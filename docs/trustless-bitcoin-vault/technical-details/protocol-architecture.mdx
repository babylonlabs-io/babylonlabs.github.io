...
The Trustless Bitcoin Vault (TBV) protocol has three layers: Bitcoin scripts and transactions, Ethereum contracts, and off-chain participant software. The Bitcoin and Ethereum layers are shared across applications. For the trust model, see [Safety & trust assumptions](../start-here/safety-and-trust-assumptions.mdx).
...
The vault output is surrounded by a pre-signed transaction graph. Every legitimate spend path is constructed and signed by the required participants at vault creation. After creation, no party can invent a new spend; Bitcoin will accept only the paths already encoded and signed.
...
* **Participants.** The depositor, Vault Provider, Application Vault Keepers, and Universal Challengers each run vault management software. The Security Council holds emergency multisig keys (3 of 5 quorum on the public testnet). Per-actor responsibilities are on [Protocol actors](./protocol-actors.mdx).
...
The app may broadcast a batched Pre-PegIn with multiple outputs, one per vault, for multi-vault peg-ins. This saves on Bitcoin fees and gives those vaults the same Bitcoin confirmation history.
...
* **Acknowledgment collection.** Each participant signs an on-chain acknowledgment. Once all are collected, the vault is ready for activation. If this does not happen before timeout (about 24 hours), the vault expires and the peg-in fee is refunded.
...
The contract verifies the hashlock and the activation timeout window (about 48 hours from creation), then notifies the registered application. For the Aave v4 integration, this deploys the depositor's position proxy if this is the first vault, mints internal `vaultBTC` accounting for the vault's BTC amount, and supplies it as collateral to the Babylon Core Spoke.
...
* The claimer broadcasts the Claim and Assert transactions. The Assert commits WOTS signatures of an invalid proof.
...
* The claimer cannot reveal a refuting secret because no valid proof exists. They cannot post a wrongly challenged response.
