...
[Babylon Trustless Bitcoin Vaults (TBV)](./what-is-tbv.mdx) are designed so that the depositor does not have to trust a third party to keep the deposited BTC safe. The BTC stays on Bitcoin, locked in a script whose spending paths are agreed at vault creation. The depositor relies on the protocol's cryptography, the Bitcoin and Ethereum networks, and the application where the collateral is used, but does not rely on a custodian, bridge operator, wrapper issuer, Vault Provider, App Keeper, Universal Challenger, or Security Council to custody the BTC.
...
![TBV trust model: depositor, Vault Provider, App Keepers, Universal Challengers, and Security Council around the Bitcoin vault](/img/trustless-bitcoin-vault/confluence/start-here/safety-and-trust-assumptions/7.png)
...
* The Bitcoin address of an arbitrageur (App Keeper), on liquidation paths.
...
* **What the depositor can do instead:** the configured App Keeper set is required during vault creation. At redemption time, the depositor can use the self-claim path with their WOTS keypair, claimer artifacts, and the pre-signed transaction graph.
* **No custody:** as with the Vault Provider, no App Keeper spend can be assembled that was not committed at vault creation.
...
* **Soft pause.** Blocks new deposits, borrows, and withdrawals. Repay, liquidate, and operations on the BTCVault Swap Spoke remain available.
...
* **BTCVault Swap Spoke liquidity dependency.** Permissionless liquidations draw WBTC from the Aave Hub. If Hub WBTC liquidity or the BTCVault Swap Spoke allowance is insufficient, permissionless liquidations revert. Liquidations can still proceed via direct redemption by registered arbitrageurs, but the pool of available liquidators narrows. The protocol mitigates this with an amount-based cap on total BTC onboarded to the Aave application.
...
On mainnet, TBV is a **capped, allowlisted Beta** with real value at stake. These caveats are load-bearing, not boilerplate:

* **Real value.** Peg-ins lock real BTC and pay real Bitcoin fees; borrowing and liquidation are economically live. There is no faucet — rehearse on the public testnet if you want a no-value dry run.
...
* **Positions and parameters are versioned per vault.** Timelocks, actor sets, and off-chain parameters are versioned in `ProtocolParams` (and per application in the Application Registry). Each vault records the parameter versions live at its creation and keeps them for its lifetime, so a later change does not retroactively alter an existing vault.
* **Allowlisted access.** Early-phase peg-ins are gated to allow-listed depositor addresses, enforced by `AllowListPolicy`. The gate is removed as launch confidence grows.
...
For the full phased-rollout picture, see [Beta Mainnet: what to expect](./beta-mainnet.mdx). For the values in force on each network, see [Networks](../network-info/networks.mdx).
...
* Wider App Keeper set per application, including additional external operators.
...
* [Beta Mainnet: what to expect](./beta-mainnet.mdx): phases, allowlisting, and caps on mainnet.
* [Community & support](../reference/community-and-support.mdx): where to report bugs and ask questions.
