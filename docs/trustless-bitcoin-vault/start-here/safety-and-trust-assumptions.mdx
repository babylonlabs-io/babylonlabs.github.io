...
* **Self-refund a stalled peg-in.** Via the refund path on the Pre-PegIn output, the short-lived Bitcoin output that holds the depositor's BTC during peg-in before activation, the depositor can recover BTC unilaterally if activation never completes.
