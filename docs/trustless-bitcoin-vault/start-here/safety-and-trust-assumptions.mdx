...
* **On-chain refund timelock.** The Pre-PegIn output includes a refund leaf with a relative timelock of 2016 BTC blocks (~14 days) for the current public-testnet parameter version. Existing vaults retain the parameter version active at creation.
...
The Pre-PegIn output has a refund leaf with a relative timelock of 2016 BTC blocks (~14 days) for the current public-testnet parameter version. If off-chain setup does not complete before `pegInAckTimeout` (approximately 24 hours), the vault expires and the peg-in fee is refunded. If setup completes but activation does not occur before `pegInActivationTimeout` (approximately 48 hours), the vault expires and the peg-in fee is non-refundable. In both cases, the depositor can sign the refund path with the depositor key and broadcast a refund transaction on Bitcoin after `tRefund` elapses. Existing vaults retain the parameter version active at creation.
