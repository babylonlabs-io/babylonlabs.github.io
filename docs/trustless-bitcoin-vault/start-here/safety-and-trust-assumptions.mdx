...
* **On-chain refund timelock.** The Pre-PegIn output includes a refund leaf with a relative timelock of 2016 BTC blocks (~14 days) on the public testnet. If activation does not complete within the configured window, the depositor can sign the refund path with the depositor key and recover BTC unilaterally.
