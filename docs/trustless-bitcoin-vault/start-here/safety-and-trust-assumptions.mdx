...
The Pre-PegIn output has a refund leaf with a relative timelock of 2016 BTC blocks (~14 days) for the current public-testnet parameter version. If activation does not occur within the activation window (approximately 48 hours), the vault enters the Expired state. The depositor can then sign the refund path with the depositor key and broadcast a refund transaction on Bitcoin to recover the BTC. Existing vaults retain the parameter version active at creation. If the vault expired because off-chain setup failed, meaning the activation window was exceeded before setup completed, the peg-in fee is also refunded.
