...
* **Smart-contract risk.** The Ethereum contracts (`BTCVaultRegistry`, `AaveAdapter`, the vaultBTC token, the Babylon Core Spoke, `CapPolicy`, `FeeEscrow`) are subject to standard smart-contract risk. Audit reports and the bug bounty program are listed on [Audits & security](../reference/audits-and-security).
...
* The demo app, the Vault Provider, and other supporting services operate on a best-effort liveness basis. Brief outages are expected.
...
* The audit posture for Testnet is tracked on [Audits & security](../reference/audits-and-security).
* Bugs and unexpected behavior should be reported through the channels listed on [Community & support](../reference/community-and-support).
