...
The Bitcoin features TBV depends on (Taproot, Schnorr signatures, hash time-locked outputs) have been part of standard Bitcoin consensus for years, with a deep track record of robust deployment.
...
TBV's design trades latency for the elimination of custodian-trust. Peg-in takes around 2 hours on signet, dominated by Bitcoin confirmations. Peg-out takes around 3 days, dominated by the challenge window during which fraud monitors can dispute a claim. These are longer than centralized wrap and unwrap services, but the depositor never gives custody to an off-Bitcoin entity. Fees are comparable in magnitude to bridge fees and lower than typical centralized wrap services.
