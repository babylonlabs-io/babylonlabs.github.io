...
Bringing Bitcoin into DeFi requires moving or representing value that lives on a chain without native smart contracts. The approaches that exist today fall into a few categories. Each category asks the depositor to accept a specific trust assumption when locking BTC. A [Trustless Bitcoin Vault (TBV)](./what-is-tbv.mdx) is an innovative approach that keeps the BTC on Bitcoin and uses cryptographic proofs to gate its release against Ethereum events. The on-Ethereum accounting token used in this approach is **vaultBTC**.
...
A TBV is a Bitcoin output behind a Taproot script. Each spending path (repayment, liquidation, fraud challenge, refund) is encoded as a separate Taproot leaf at vault creation. Every legitimate spend is constructed and signed in advance by the depositor and the protocol participants. After the vault is created, no party can fabricate a new spend; the only spends Bitcoin will accept are the ones already encoded in the script.

### Ethereum events are verified inside Bitcoin script

When a vault is redeemed, a zero-knowledge proof of the corresponding Ethereum burn event is verified inside a Bitcoin Taproot script. Verification uses the BABE construction (in the BitVM3 family) — garbled circuits and Winternitz One-Time Signature (WOTS) commitments — which fit inside Bitcoin's existing script capabilities and require no Bitcoin soft fork. The result: Bitcoin's own consensus rules enforce that BTC is released only when the matching Ethereum state has occurred.

### The depositor's keys participate in every depositor-controlled release path

The depositor's signatures are required at vault creation, at activation, and on every depositor-controlled release path. A self-claim path using the depositor's WOTS key (plus claimer artifacts downloaded at peg-in) is committed at vault creation, which lets the depositor exit independently if the Vault Provider is offline or unresponsive. A refund timelock on the Pre-PegIn output recovers the BTC if activation never completes. Details on these recovery paths are on [Safety & trust assumptions](./safety-and-trust-assumptions.mdx).
...
| What represents the BTC on the destination chain | Custodian-issued token | Bridge-minted token | `vaultBTC` accounting token |
...
* **BitVM-style constructions (2023+)** showed how to verify arbitrary off-chain computation using only existing Bitcoin script primitives — hashes, signatures, and timelocks. The BABE construction is in this family; the cut-and-choose ensures malicious construction is detectable.
* **Practical ZK proof systems (SP1, 2024+)** made it operationally feasible to prove an Ethereum state change quickly enough — 5-10 minutes on GPU hardware for a single vault — and compress the proof to a few hundred bytes for Bitcoin verification.

The Bitcoin features TBV depends on — Taproot, Schnorr signatures, hash time-locked contracts — have been part of standard Bitcoin consensus for years and have a deep track record of robust deployment.
...
TBV's design trades latency for the elimination of custodian-trust. Peg-in takes around 2 hours on signet, dominated by Bitcoin confirmations. Peg-out takes around 3 days, dominated by the challenge window during which fraud monitors can dispute a claim. These are longer than centralized wrap/unwrap services, but the depositor never gives custody to an off-Bitcoin entity. Fees are comparable in magnitude to bridge fees and lower than typical centralized wrap services.
