...
The depositor's signatures are required at vault creation, at activation, and on every depositor-controlled release path. A self-claim path using the depositor's WOTS key (plus claimer artifacts downloaded at peg-in) is committed at vault creation, which lets the depositor exit independently if the Vault Provider is offline or unresponsive. A refund timelock on the Pre-PegIn output recovers the BTC if activation never completes. Details on these recovery paths are on [Safety & trust assumptions](./safety-and-trust-assumptions).
