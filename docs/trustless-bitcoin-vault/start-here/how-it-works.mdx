...
An active vault is automatically supplied as collateral on the application chosen at peg-in. On activation, the BTC Vault Manager notifies the Aave v4 Adapter. The adapter creates an internal **collateral record** of the vault's BTC amount and supplies it to the **Babylon Core Lending Spoke**, the dedicated Aave v4 market that accepts it as collateral. The vault is appended to the depositor's borrowing position; subsequent activations add to the same position.
