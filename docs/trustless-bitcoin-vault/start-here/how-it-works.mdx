...
A **BTCVault** spans two networks. BTC stays on the Bitcoin Network for the vault's entire lifetime. On Ethereum, a protocol contract tracks each vault's state and registers DeFi applications such as Aave v4. The trust assumption is cryptographic evidence: no bridge, no off-chain oracle, no wrapping custodian.

:::note This flow is the same on both networks
The mechanics below apply to mainnet and to the public testnet alike. On mainnet it is a **real-value** flow: peg-ins lock real BTC and pay real Bitcoin fees, and borrowing and liquidation are economically live. Exposure is also capped from day one — `CapPolicy` enforces a per-application total (`totalCapBTC`) and a per-address cap (`perAddressCapBTC`), and `ProtocolParams` enforces a per-vault maximum (`maxPegInAmount`). Timing figures below are testnet values; mainnet challenge windows, confirmation depths, and cap values are **to be announced at launch**. See [Networks](../network-info/networks.mdx) and [Beta Mainnet: what to expect](./beta-mainnet.mdx).
:::
...
While the Pre-PegIn transaction accumulates Bitcoin confirmations, the depositor and the App Keepers (off-chain participants that co-sign the vault's transaction graph; none of them have custody of the depositor's BTC) complete an off-chain setup. They construct the vault's transaction graph: the complete set of pre-signed transactions covering every legitimate spend path (each redemption path, challenge response, and the Pre-PegIn refund). Each participant signs their part.
