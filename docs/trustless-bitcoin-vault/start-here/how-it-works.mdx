...
* **Refund relative timelock.** If the peg-in stalls (the depositor never reveals the secret, or participants fail to complete setup), the depositor recovers the BTC unilaterally via a Bitcoin-side relative CSV timelock on the Pre-PegIn output.
