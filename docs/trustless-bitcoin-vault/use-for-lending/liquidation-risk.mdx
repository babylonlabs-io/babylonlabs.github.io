...
* **Permissionless (LLP-routed).** The liquidator repays the debt and is settled instantly in a fungible wrapped-BTC asset, WBTC at launch, drawn from the LLP plus the liquidation bonus. The seized vault enters LLP escrow. The first LLP is the **BTC Vault Swap Spoke** (the `BTCVaultSwap` contract), registered as its own WBTC spoke on the Aave v4 Hub.
