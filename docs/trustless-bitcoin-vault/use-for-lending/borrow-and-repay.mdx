...
The collateral factor for BTC collateral on the public testnet is listed in [Protocol parameters](../testnet-info/protocol-parameters.mdx). Since the collateral record is the only collateral on the Babylon Core Lending Spoke, the position's collateral factor is constant (positions on other spokes with heterogeneous collateral would use a weighted average).
