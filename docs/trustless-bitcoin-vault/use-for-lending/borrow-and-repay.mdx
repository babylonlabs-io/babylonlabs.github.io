...
Public-testnet caps on per-address total collateral and per-position vault count apply. See [Protocol parameters](../network-info/testnet/protocol-parameters.mdx) for the live numeric limits.
:::
...
The collateral factor for BTC collateral on the public testnet is listed in [Protocol parameters](../network-info/testnet/protocol-parameters.mdx). Since the collateral record is the only collateral on the Babylon Core Lending Spoke, the position's collateral factor is constant (positions on other spokes with heterogeneous collateral would use a weighted average).
...
Position cap reminder: public-testnet per-position and per-address caps apply. See [Protocol parameters](../network-info/testnet/protocol-parameters.mdx) for the current values.
