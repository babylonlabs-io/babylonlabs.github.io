...
The public testnet enforces conservative limits on individual vaults, positions, and total application exposure. These limits will grow as the protocol matures.
...
Per-application and per-address BTC exposure limits enforced at vault activation. These public-testnet limits will grow as the protocol matures.

| Parameter | Value | Description |
| --- | --- | --- |
| `totalCapBTC` (Aave application) | 10 BTC | Maximum total BTC across all users for the Aave v4 application |
| `perAddressCapBTC` (Aave application) | 0.4 BTC | Maximum BTC per address for the Aave v4 application |
...
| `feeRate` | 2 sat/vB | Bitcoin transaction fee rate used by the protocol (public testnet; mainnet is higher). |
...
| Transfer restrictions | Restricted to the Aave v4 integration; non-transferable and non-fungible | vaultBTC is not freely transferable. It exists only as an internal accounting unit within the Aave Adapter. |

## Related

* [TBV Glossary](../reference/glossary.mdx): term definitions for the parameters above.
