...
| Refund timelock | ~14 days (2016 BTC blocks) |
...
| `tRefund` | 2016 BTC blocks (~14 days) | Pre-PegIn refund timelock (relative, `OP_CHECKSEQUENCEVERIFY` from Pre-PegIn confirmation). After this elapses, the depositor can sign the refund path on Bitcoin to reclaim the BTC if peg-in stalled. |
