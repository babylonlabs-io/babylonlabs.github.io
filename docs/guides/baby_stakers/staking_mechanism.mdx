...
- Use [Babylon Node](/operators/babylon_node/installation_guide/) or [RPC Endpoints](/developers/babylon_genesis_chain/#network-information) to query the `LastEpochMsgs` endpoint in the
  `x/epoching` module.
