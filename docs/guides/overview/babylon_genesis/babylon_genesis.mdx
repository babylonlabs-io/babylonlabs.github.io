...
Babylon Genesis is the coordination layer for the Babylon protocol, serving as the
control plane for Bitcoin staking security and liquidity orchestration.
...
1. Enables massive economic security from Bitcoin to secure proof-of-stake networks
2. Plays a key role in coordinating staking operations and reward distribution
...
- **Bitcoin Staking:** Enables Bitcoin holders to stake and earn rewards
...
Babylon Genesis is governed by the native token BABY. BABY holders can actively engage in the decision-making process, influencing the direction and development of the protocol. This participatory model ensures that the network always reflects the collective will of its token holders.

Proposals are critical function in the governance process. These proposals can cover a range of topics, from software upgrades to smart contract deployments during the permissioned phase, and protocol parameter changes. It is a structured framework for submitting, discussing, and voting on these proposals, ensuring transparency and accountability.
...
:::note
Validators are accepted on the Babylon Genesis network. The network is currently in
testnet.
:::

<Link
  to="https://github.com/babylonlabs-io/networks/tree/main/bbn-test-6/babylon-validators"
  id="validators"
>
    Become a Validator
</Link>
...
:::note
Finality Providers are accepted on the Babylon Genesis network.
:::
