...
After you initiate the unbonding process, you need to wait for a specific period to complete the unbonding. Specifically, you must wait for 301 Bitcoin blocks (typically ~2 days, but can take up to a week depending on network conditions) to be mined. This waiting period ensures the stability and security of the network. Note that in rare cases, network conditions could cause slight delays.
