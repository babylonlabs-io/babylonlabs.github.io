...
example, for Babylon testnet, the chain ID is `bbn-test-6`.
5. To change the Babylon RPC/GRPC address, update the following:

 ```bash
 RPCAddr = https://rpc.testnet.babylonlabs.io:443 # rpc node address
 GRPCAddr = https://grpc.testnet.babylonlabs.io:443 # grpc node address
    ```
The above addresses are for Babylon testnet.
