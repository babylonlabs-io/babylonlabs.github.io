...
This page provides a guide for PoS systems to integrate with Euphrates 0.6.0. The integration allows a BSN to get economic security from BTC staking.
...
- **A BSN (sometimes referred to as a consumer system in code).** This is the system that we aim to integrate with Babylon Bitcoin Staking Protocol.

**User stories.** The integration provides the following user stories for BSNs:

1. Babylon Genesis receives registrations of BSNs, BSN Finality Providers, and BTC delegations (shipped in Euphrates v0.1.0).
2. Babylon Genesis propagates BSN Finality Providers and BTC delegations to BSNs (shipped in Euphrates v0.2.0).
3. BSN Finality Providers submit finality signatures to their BSNs (shipped in Euphrates v0.3.0).
4. BSNs tally blocks and determine their finalization status (shipped in Euphrates v0.4.0).
...
The BSN integrating with Euphrates 0.6.0 needs to support IBC and CosmWasm smart contracts. This is because Babylon sends information about Finality Providers and BTC delegations as IBC packets. The BSN needs to deploy CosmWasm smart contracts to process these IBC packets, and then store the Finality Providers and BTC delegations information.
...
For Cosmos-based blockchains, one needs to add the `x/babylon` [module](https://github.com/babylonlabs-io/babylon-sdk/tree/main/x) from the [Babylon SDK](https://github.com/babylonlabs-io/babylon-sdk) to the BSN blockchain code base. Other BSNs are not supported at the moment.
...
The BSN will return the `<babylon_contract_address>` of the Babylon contract.
...
Starting with this version v0.5.0, the BSN is registered automatically during IBC channel establishment. That’s what the `consumer_name` and `consumer_description` parameters of the Babylon Contract instantiation message are for.
...
### Become a Finality Provider for your BSN
...
**Note**: Mind to use your BSN id as `chain-id` for the CLI commands.
...
You can register a created Finality Provider in Babylon through the `fpd register-finality-provider`. Note that one needs to obtain some tBABY tokens from [Babylon discord channel](https://discord.com/invite/babylonglobal) first. The output contains the hash of the Babylon Finality Provider registration transaction.
...
**Staking BTC to your Finality Providers.** When staking, specify the BTC public keys of Finality Providers using the `--finality-providers-pks` flag in the `stake` command. Note that **Babylon requires a BTC delegation to restake to at least 1 Babylon Finality Provider, apart from Finality Providers of other BSNs.**
...
### Wait until BSN receives IBC packets

After some time, the Babylon contract on the BSN side is supposed to receive IBC packets about these Finality Providers and BTC delegations, and store them in the local KV store. If the BSN is based on Cosmos SDK, then you can query Finality Providers via
...
Once the BSN receives IBC packets of Finality Providers and BTC delegations, the Finality Provider daemon will automatically start submitting finality signatures to the BTC staking contract on the BSN. One can query the finality signature at a given block signed by a given Finality Provider via the following query
...
In this iteration, Babylon supports reward distribution that occurs in the BSN.
