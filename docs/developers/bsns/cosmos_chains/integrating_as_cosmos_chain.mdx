...
This page provides a guide for PoS systems to integrate Babylon phase-3 devnet, called Euphrates. The integration allows a BSN to get economic security from BTC staking.
...
To integrate with Euphrates devnet, you need to install and set up the following software:

### Golang / Rust

This project requires Go version 1.23 and Rust `stable` version. You can install Go by following [https://go.dev/doc/install](https://go.dev/doc/install), and install Rust by following [https://www.rust-lang.org/tools/install](https://www.rust-lang.org/tools/install).
...
```bash
$ bcd query babylon params
```

to get the code IDs and addresses of the Babylon contracts.
