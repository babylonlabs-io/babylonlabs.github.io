...
Adjust this configuration according to your setup, particularly changing the placeholder values (like `<BSN_id>`, `<BSN_system_rpc_addr>`, etc.) to match your specific BSN configuration.
...
The Babylon contracts distribute rewards to Finality Providers upon each `EndBlock`. 

You can query the reward distribution via:

```bash
$ babylond query incentive reward-gauges <your-bech32-address> # of finality provider 
```

To withdraw the reward as a Finality Provider:
